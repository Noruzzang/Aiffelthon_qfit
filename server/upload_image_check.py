...
#----------------------------------------------------------------------------#
# upload_image 폴더에서 파일 이름을 읽어 그룹화된 데이터를 생성하는 함수
#----------------------------------------------------------------------------#
    ...
    try:
        ...
        for index, image_filename in enumerate(result):         
            ...
            try: 
                ...
                #------------------------------------------------------#
                pymunk_sim6_file = os.path.join(home_dir, "aiffelthon_qfit", "model_src", "pymunk_sim6.py")
                
                pym_result = []
                try:
                    logger.info(f"==== [ pymunk_sim6.py 스크립트 실행 ] ====") 
                    pym_result = subprocess.run(["python", pymunk_sim6_file], check=True)
                    logger.info(f"pym_result: {pym_result}")
                    ...
                    if index == len(result) - 1: #마지막 파일이면
                        ...
                        #--------------------------------------------------------------------------------#
                        logger.info(f"==== [ upload_image 폴더의 정보로 리턴값 생성 시작 ]  ====")
                             
                        data = generate_data_from_folder(dest_folder)
                        logger.info(f"data: {data}")
                        
                        # JSON 파일로 저장
                        json_file_path = save_json_to_folder(data, dest_folder, current_time)
                        
                        # 결과 로그
                        logger.info(f"===== [ JSON 파일 저장 완료: {json_file_path} ] ====")
                        
                        logger.info(f"==== [ 리턴할 result 값 생성 완료 ]  ====")
                    ...
                    logger.error(f"pymunk_sim6 스크립트 실행 중 오류 발생: {e}")
                    return {"status": "error", "message": f"pymunk_sim6 Script execution error: {str(e)}"}                  
                                
            except subprocess.CalledProcessError as e:
                logger.error(f"topview 스크립트 실행 중 오류 발생: {e}")
                return {"status": "error", "message": f"topview Script execution error: {str(e)}"}  
        
    except FileNotFoundError as e:
        logger.error(f"{e}")
        return {"status": "error", "message": "No image files found in the folder."}
    except subprocess.CalledProcessError as e:
