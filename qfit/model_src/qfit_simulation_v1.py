...
def overlay_frame(base_image, frame_image_path):
    """
    base_image: 공과 궤적이 표시된 당구대 이미지 (BGR 형식)
    frame_image_path: 당구대 외곽 프레임 이미지 경로 (BGRA 포맷, 투명도 포함)

    이 함수를 사용하여 당구대 위에 외곽 프레임을 합성하고 최종 출력을 생성합니다.
    """
    ...
    x_offset = (w_f - w_b) // 2
    y_offset = (h_f - h_b) // 2
    # base_image를 frame 중앙에 복사
    result[y_offset : y_offset + h_b, x_offset : x_offset + w_b, :3] = base_image

    # 알파 채널로 블렌딩
    if frame_image.shape[2] == 4:
        alpha = frame_image[:, :, 3] / 255.0
        alpha = cv2.merge((alpha, alpha, alpha))
        frame_bgr = frame_image[:, :, :3].astype(float)
        result_bgr = result[:, :, :3].astype(float)
        alpha_f = alpha.astype(float)

        out = alpha_f * frame_bgr + (1.0 - alpha_f) * result_bgr
        out = np.clip(out, 0, 255).astype(np.uint8)
        return out
    else:
        return result

def load_front_ball_texture():
    """
    정면에서 본 흰색 큐볼(white cue ball) 이미지를 불러옵니다.
    """
    front_ball_texture = cv2.imread(front_ball_path, cv2.IMREAD_UNCHANGED)
    if front_ball_texture is None:
        print(f"[오류] 정면 큐볼 이미지 불러오기 실패: {front_ball_path}")
        return None
    return front_ball_texture
...
def save_power_gauge_image(power_gauge):
    ...
    text_size = draw.textbbox((0, 0), text, font = font)  # 텍스트 크기 측정
    text_width = text_size[2] - text_size[0]

    text_x = bar_w // 2 - text_width // 2
    text_y = bar_h // 2 - 10  # 중앙 정렬

    draw.text((text_x, text_y), text, font = font, fill = (255, 255, 255, 255))  # 흰색 텍스트

    # 최종 이미지 변환 (투명 배경 유지)
    image = np.array(image_pil)

    success = cv2.imwrite(gauge_image_path, cv2.cvtColor(image, cv2.COLOR_RGBA2BGRA))  # RGBA를 BGRA로 변환 후 저장
    if success:
        logger.info(f"파워 게이지 이미지: [{gauge_image_path}] 저장") 
    else:
        logger.info(f"[오류] 파워 게이지 이미지 저장 실패") 
    return success

def show_power_gauge_image(power_gauge):
    """
    저장된 파워 게이지 이미지를 불러와 별도로 표시
    """
    ...
    image_rgb = cv2.cvtColor(image, cv2.COLOR_BGRA2RGBA)  # BGRA → RGBA 변환
...
# (F) 시각화 함수(강도 바 포함)
############################################################################
def draw_power_bar(image, power_gauge):
    # 파워를 0~10 사이로 clamp
    power_gauge = min(power_gauge, 10.0)
    if power_gauge < 0:
        power_gauge = 0
    ...
    bar_h = 20
    x0,y0 = 20,20
    cv2.rectangle(image, (x0, y0), (x0 + bar_w, y0 + bar_h), (80, 80, 80), -1)
    fill_w = int(bar_w * (power_gauge / 10))
    cv2.rectangle(image, (x0, y0), (x0 + fill_w , y0 + bar_h), (0, 200, 0), -1)
    text = f"{power_gauge} / 10"
    cv2.putText(image, text, (x0 + bar_w + 5, y0 + bar_h - 5),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)

def draw_trajectory_on_table(table_image, traj, power_gauge = None):
    """
    ...
    for cname, points in traj.items():
        ...
        for i in range(0, len(points), step):
            x, y = points[i]
            cx, cy = int(x), int(y)  # 정수 변환
            cv2.circle(trajectory_image, (cx, cy), 3, color_bgr, -1)

    # 파워 게이지 추가 (선택적)
    if power_gauge is not None:
        draw_power_bar(trajectory_image, power_gauge)

    return trajectory_image


############################################################################
# (G) 정면에서 본 공 + 히트 포인트 표시
############################################################################
# 폰트 경로 설정
font_path = os.path.join(home_dir, "aiffelthon_qfit", "model_src", "image", "Roboto-Regular.ttf")

def show_front_hit_point(angle_deg, offset_xy):
    """
    정면에서 본 흰색 공 위에 히트 포인트와 3D 텍스트('Hit Here')를 추가합니다.
    """
    w, h = 200, 200
    center = (w // 2, h // 2)
    radius = 80
    ...
    # 배경을 RGBA(투명)로 생성
    front_view_image = np.full((h, w, 4), (220, 220, 220, 0), dtype = np.uint8)  # 투명 배경
    ...
    # 큐볼 이미지 로드
    front_ball_path = os.path.join(home_dir, "aiffelthon_qfit", "model_src", "image", "white_cue_ball.png")   
    front_ball_texture = cv2.imread(front_ball_path, cv2.IMREAD_UNCHANGED)
    
    if front_ball_texture is not None:
        front_ball_texture = cv2.resize(front_ball_texture, (2 * radius, 2 * radius),
                                      interpolation = cv2.INTER_AREA)
        x1, y1 = center[0] - radius, center[1] - radius
        x2, y2 = center[0] + radius, center[1] + radius

        # 알파 채널 합성 (큐볼만 표시하고 배경 유지)
        alpha_s = front_ball_texture[:, :, 3] / 255.0
        alpha_l = 1.0 - alpha_s
        for c in range(3):
            front_view_image[y1:y2, x1:x2, c] = (alpha_s * front_ball_texture[:, :, c]
                                                 + alpha_l * front_view_image[y1:y2, x1:x2, c])
        front_view_image[y1:y2, x1:x2, 3] = (alpha_s * 255).astype(np.uint8)  # 투명도 적용
    ...
    if hit_point_image is not None:
        dot_size = int(radius * 0.3)  # 공 크기의 30%로 조정
        ...
        if hit_point_image.shape[2] == 4:
            alpha_s = hit_point_image[:, :, 3] / 255.0
            alpha_l = 1.0 - alpha_s
            for c in range(3):
                front_view_image[y1:y2, x1:x2, c] = (alpha_s * hit_point_image[:, :, c]
                                                     + alpha_l * front_view_image[y1:y2, x1:x2, c])
            front_view_image[y1:y2, x1:x2, 3] = (alpha_s * 255).astype(np.uint8)  # 투명도 유지

    # Pillow(PIL)를 사용하여 3D 효과 적용한 "Hit Here" 텍스트 추가
    image_pil = Image.fromarray(front_view_image)
    draw = ImageDraw.Draw(image_pil)

    try:
        font = ImageFont.truetype(font_path, 24)  # Roboto-Regular.ttf 폰트 적용
    except IOError:
        print(f"[오류] 폰트 파일 찾기 실패: {font_path}")
        font = ImageFont.load_default()  # 기본 폰트 사용

    # 텍스트 크기 측정 (중앙 정렬을 위해 필요)
    text_size = draw.textbbox((0, 0), "Hit Here", font = font)
    text_width = text_size[2] - text_size[0]  # 텍스트 너비

    # 중앙 정렬을 위해 텍스트 위치 조정
    text_offset_y = -70
    text_position = (tx - text_width // 2, max(ty + text_offset_y, 5))  # 중앙 정렬 + 최소 높이 유지

    # 3D 효과 적용 (검은 그림자)
    shadow_offset = (2, 2)  # 그림자 위치 조정
    draw.text((text_position[0] + shadow_offset[0], text_position[1] + shadow_offset[1]),
            "Hit Here", font = font, fill = (50, 50, 50))  # 검은 그림자

    # 기본 텍스트 (흰색)
    draw.text(text_position, "Hit Here", font = font, fill = (255, 255, 255))

    # 최종 이미지 변환 (투명 배경 포함)
    front_view_image = np.array(image_pil)

    # OpenCV에서 알파 채널 유지 후 RGB 변환
    out_rgb = cv2.cvtColor(front_view_image, cv2.COLOR_BGRA2RGBA)
    ...
    return front_view_image  # PNG 저장 가능
...
def main():
    ...
    out_rgb = cv2.cvtColor(overlaid_image, cv2.COLOR_BGR2RGB)
    plt.figure(figsize = (10, 5))
    plt.imshow(out_rgb)
    plt.title(f"Best Shot : A = {best_angle}, P = {best_power}, Off = {best_offset}, Score = {best_score} (Frame Overlay)")
    plt.axis("off")
    plt.show()

    # Best-Shot 저장
    cv2.imwrite(best_shot_path, overlaid_image)
    logger.info(f"Best Shot 이미지: [{best_shot_path}] 저장")  
    

    # 3) 정면 타격 지점 (히트 포인트 + 'Hit Here')
