...
class SelectPage extends StatelessWidget {
                          ...
                          child: Column(
                            ...
                            children: [
                              if (cameraImagePath != null)
                                _buildImageSection('images/cameraImage_txt.png', cameraImagePath!, context),

                              if (galleryImagePath != null)
                                _buildImageSection('images/galleryImage_txt.png', galleryImagePath!, context),
                            ],
                          ),
                        ),
                      ),
  ...
  Widget _buildImageSection(String titleImage, String imagePath, BuildContext context) {
    return FutureBuilder<img.Image>( //비동기 작업(Future)이 완료될 때까지 대기 (완료되면 snapshot.data에 이미지가 저장됨)
      future: _loadImage(File(imagePath)),  //비동기적으로 이미지 로드
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.done && snapshot.hasData) { //이미지 로딩이 완료되었고, 데이터가 있는 경우 UI 렌더링 진행
          final img.Image image = snapshot.data!;
          final bool isPortrait = image.height > image.width;  //세로 이미지인지 확인(true이면 세로 이미지, false이면 가로 이미지)

          return Column( //세로 방향으로 요소 배치
            crossAxisAlignment: CrossAxisAlignment.start, //왼쪽 정렬
            children: [
              Image.asset(titleImage, width: 170), //제목(타이틀) 이미지를 표시하기 위한 이미지 경로(assets 폴더에서 불러와 표시)
              const SizedBox(height: 10), //간격 추가
              Container(
                decoration: BoxDecoration( //그림자 효과 적용
                  boxShadow: [
                    BoxShadow(
  ...
  Future<Map<String, String>> _uploadImage(String imagePath) async {
    ...
    //요청을 보낼 서버의 URL을 설정
    var uri = Uri.parse('https://1356-112-172-64-10.ngrok-free.app/upload_image/');
    ...
    try {
      ...
      //서버 응답 처리
      if (response.statusCode == 200) { //요청 성공 여부 확인
        //응답 데이터를 문자열로 변환
        ...
        if (decodedData.containsKey('data') && decodedData['data'].isNotEmpty) {
          ...
          for (var item in decodedData['data']) {
            ...
            if (item.containsKey('upload_image') && item['upload_image'] != null) {
            ...
            if (item.containsKey('power_gauge') && item['power_gauge'] != null) {
          ...
          // Map 형식으로 결과 반환
          return {'best_shot': bestShot, 'front_view': frontView, 'power_gauge': powerGauge, 'image_prefix': prefix};
              ...
              if (cameraImagePath != null) {
                ...
                if (photo != null) {
                  Navigator.pushReplacement(
                    context,
                    MaterialPageRoute(builder: (context) => SelectPage(cameraImagePath: photo.path)),
                  );
                }
                ...
                if (image != null) {
                  Navigator.pushReplacement(
                    context,
                    MaterialPageRoute(builder: (context) => SelectPage(galleryImagePath: image.path)),
                  );
                }
              }
            },
          ...
          IconButton(
            onPressed: () async {
              ...
              try {
                ...
                //============================================================================
                print('cameraImagePath : $cameraImagePath, galleryImagePath: $galleryImagePath');

                Map<String, String> result = await _uploadImage(cameraImagePath ?? galleryImagePath!);
                print('result: $result');

                // 'best_shot' 키가 없거나 null이면 예외 처리
                if (!result.containsKey('best_shot') || result['best_shot'] == null) {
                  throw Exception("best_shot 이미지 URL을 찾을 수 없습니다.");
                }

                // 'image_prefix' 키가 없거나 null이면 예외 처리
                if (!result.containsKey('image_prefix') || result['image_prefix'] == null) {
                ...
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => ResultPage( //api호출후 받은 당구관련 이미지 3개를 보내서 화면에 표시
            ...
            icon: Image.asset('images/icon/search_icon.png', width: 60, height: 60),
          ),
          const SizedBox(height: 40),
          GestureDetector(
            onTap: () {
