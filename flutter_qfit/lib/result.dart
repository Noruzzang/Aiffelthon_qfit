...
class ResultPage extends StatefulWidget {
  //StatelessWidget {
  //Map<String, String> result; //upload_image api 호출후 받은 3개의 이미지파일정보
...
class _ResultPageState extends State<ResultPage> {
  ...
  //Map<String, dynamic> imageUrls = {}; // 받아온 이미지 URL 저장

  String bestShotImage = '';  // Best Shot 이미지 URL 저장
  String frontViewImage = ''; // Front View 이미지 URL 저장

  Uint8List? bestShotImage1;
  Uint8List? frontViewImage1;

  @override
  void initState() {
    super.initState();
    fetchAndSetImageUrls(); // 이미지 URL 가져오기
    //fetchData(widget.imagePrefix); // 이미지 URL 가져오기
    //fetchImages();
  }

  Future<void> fetchImages() async {
    try {
      String baseUrl = "https://84d3-112-172-64-10.ngrok-free.app";
      bestShotImage1 = await fetchImage("$baseUrl/image/${widget.imagePrefix}_best_shot.png");
      frontViewImage1 = await fetchImage("$baseUrl/image/${widget.imagePrefix}_front_view.png");
      setState(() {});  // UI 업데이트
    } catch (e) {
      print("이미지 불러오기 오류: $e");
    }
  ...
  Future<Uint8List?> fetchImage(String url) async {
    ...
    if (response.statusCode == 200) {
      return response.bodyBytes; // 이미지 바이너리 데이터 반환
    } else {
  ...
  Future<void> fetchData(String imagePrefix) async {
    try {
      var enteredUrl = 'https://84d3-112-172-64-10.ngrok-free.app/image_info/'; // FastAPI 서버 주소
      print("api 호출요청: $enteredUrl$imagePrefix");
      
      var response = await http.get(
        Uri.parse(enteredUrl + imagePrefix),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          if (data.containsKey("image_urls")) {
            //bestShotImage = data['image_urls']['best_shot'] ?? '';
            bestShotImage = data['image_urls']['best_shot'];
            print('bestShotImage: $bestShotImage');
            frontViewImage = data['image_urls']['front_view'];
            print('frontViewImage: $frontViewImage');
          }
        });

        // 사용 예제
        checkImageUrl(bestShotImage);
        checkImageUrl(frontViewImage);
  ...
  void checkImageUrl(String url) async {
    final response = await http.get(Uri.parse(url));
    if (response.statusCode == 200) {
      print("Valid image URL: $url");
    } else {
      print("Invalid image URL: $url (Status: ${response.statusCode})");
    }
  }
  ...
  //Future<void> fetchImageUrls(String imagePrefix) async {
    try {
      var apiBaseUrl = 'https://84d3-112-172-64-10.ngrok-free.app/image_info/'; // FastAPI 서버 주소
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            image: DecorationImage( //예측 당점 이미지
                              /*image: frontViewImage1 != null //frontViewImage.isNotEmpty
                                  ? MemoryImage(frontViewImage1!) //NetworkImage(frontViewImage) // FastAPI에서 받은 이미지 적용
                                  : AssetImage('images/jellyfish.jpg') as ImageProvider,// 기본 이미지 */
                              image: frontViewImage.isNotEmpty
                                  ? NetworkImage(frontViewImage) // FastAPI에서 받은 이미지 적용
                                  : AssetImage('images/front_view.png') as ImageProvider, // 기본 이미지
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            image: DecorationImage( //파워 게이지 이미지 (width: 250, height: 318)
                              /*image: frontViewImage1 != null //frontViewImage.isNotEmpty
                                  ? MemoryImage(frontViewImage1!) //NetworkImage(frontViewImage) // FastAPI에서 받은 이미지 적용
                                  : AssetImage('images/jellyfish.jpg') as ImageProvider, // 임시 이미지*/
                              image: frontViewImage.isNotEmpty
                                  ? NetworkImage(frontViewImage) // FastAPI에서 받은 이미지 적용
                                  : AssetImage('images/power_gauge.png') as ImageProvider, // 기본 이미지
                              fit: BoxFit.scaleDown, // BoxFit을 사용하여 이미지를 맞춤
                            ),
                          ),
                          width: 170, // 이미지의 가로 크기 설정
              ...
              Expanded(
                ...
                child: Center(  // Center 위젯 추가하여 중앙 배치
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      image: DecorationImage(
                        /*image: bestShotImage1 != null //bestShotImage.isNotEmpty
                          ? MemoryImage(bestShotImage1!) //NetworkImage(bestShotImage) // FastAPI에서 받은 이미지 적용
                            : AssetImage('images/with_ball.png') as ImageProvider, // 임시 이미지*/
                        image: bestShotImage.isNotEmpty
                            ? NetworkImage(bestShotImage)
                            : AssetImage('images/best_shot.png'), // 임시 이미지
                        fit: BoxFit.scaleDown, // BoxFit을 사용하여 이미지를 맞춤
                      ),
                    ),
